...
public class ItextTable extends ItextObjectImpl {
	...
	private Collection<ItextCell>	cells;
	...
	public ItextTable() {
		...
		this.cells = new ArrayList<ItextCell>();
	}
		...
		if (this.totalWidth > 0) {
			...
			if (this.cells.size() > 0) {
				...
				for (int i = 0; i < this.columns; i++) {
					ItextCell cell = ((ArrayList<ItextCell>) this.cells).get(i);
					if (cell.getWidth() > 0) {
						ok = true;
	...
	public void addCell(ItextCell cell) {
		this.cells.add(cell);
	}

	public Collection<ItextCell> getCells() {
		return this.cells;
	}

	public void removeCell(int index) {
		((ArrayList<ItextCell>) this.cells).remove(index);
	}

	public void removeCell(ItextCell cell) {
		this.cells.remove(cell);
	}

	public void setCells(Collection<ItextCell> cells) {
		if (this.cells.size() > 0) {
			while (this.cells.size() > 0) {
				((ArrayList<ItextCell>) this.cells).remove(0);
			}
		}

		for (ItextCell cell : cells) {
			this.cells.add(cell);
		}
